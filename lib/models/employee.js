...
class Employee extends Model {
    ...
    updateEmployeeRole(employeeId, roleId) {
        const sql = `UPDATE ${this.table} SET role_id = ${roleId} WHERE id = ${employeeId}`;
        try {
            this.query(sql);
        } catch (error) {
            error ? console.log(error) : console.log(`Updated employee id ${employeeId} to role id ${roleId}.`);
        }
